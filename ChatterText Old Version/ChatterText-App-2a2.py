...
# -*- coding: utf-8 -*-
"""
ChatterText - App Desktop Python v2.2
Posizionare nella root di Chatterbox ed eseguire con: python chattertext_app.py

NOVITA v2.2:
  - NUOVE pause [bd] ~1.60s e [cap] ~2.00s per narrativa/filosofia
  - Tutte le pause usano random.gauss(base, sigma) -> naturalezza anti-robotica
  - NUOVE giunzioni [stacco] 1.40s e [lungo] 1.80s (fade+sil+fade)
  - dynamic_pause() avanzata: scala 6 livelli, gauss, moltiplicatori semantici
    (filosofia, concetti, riflessione, dialogo, lunghezza testo)
  - Prompt Guida aggiornato con tutti i nuovi tag

SCALA PAUSE INLINE:
  [p1]  ~0.18s +/-0.03  micro      virgola, incisi
  [p2]  ~0.40s +/-0.05  breve      punto normale
  [p3]  ~0.65s +/-0.07  media      riflessione, ;
  [b]   ~1.00s +/-0.10  lunga      cambio idea, :
  [bd]  ~1.60s +/-0.15  drammatica climax, rivelazioni  [NUOVO]
  [cap] ~2.00s +/-0.20  capoverso  reset mentale        [NUOVO]
...
GIUNZIONI (7 livelli):
  ...
  [stacco] 1.40s fade+sil+fade cambio pensiero/concetto [NUOVO]
  [lungo]  1.80s fade+sil+fade pausa riflessiva         [NUOVO]
  [scena]  2.00s hard          cambio scena/capitolo
"""
...
# PROMPT GUIDA
# =========================================================
GUIDE_PROMPT = '''# PROMPT PER RISCRITTURA CAPITOLO — ChatterText TTS v2.2

Sei un editor specializzato nella preparazione di testi per la sintesi vocale con Chatterbox TTS.
Riscrivi il capitolo che ti fornirò applicando il sistema di tag di ChatterText v2.2.
...
================================================================================
## TAG PAUSE INLINE — SCALA A 6 LIVELLI  (con variazione gaussiana automatica)
================================================================================
  ...
  [bd]  ~1.60s  PAUSA DRAMMATICA  [NUOVO v2.2]
        -> rivelazioni, climax, chiusure forti, suspense
        -> "potentissima, usare con parsimonia"
        -> dopo: "Ed era proprio questo la verita."

  [cap] ~2.00s  CAPOVERSO / RESET MENTALE  [NUOVO v2.2]
...
REGOLA D'ORO: meglio una pausa piu lunga che troppo corta.
Il cervello ha bisogno di tempo per "sentire", specie in filosofia e narrativa.
...
## TAG DI GIUNZIONE — 7 LIVELLI  (obbligatorio prima di [fine])
   ...
   Diversi dalle pause inline: agiscono sulla fusione audio tra chunk contigui.
================================================================================
  ...
  [stacco]  1.40s  fade+sil+fade  CAMBIO PENSIERO: concetto diverso     [NUOVO v2.2]
  [lungo]   1.80s  fade+sil+fade  PAUSA RIFLESSIVA: sospensione teatrale [NUOVO v2.2]
  [scena]   2.00s  stacco secco   cambio scena, salto temporale
...
NOTA — pause inline vs giunzioni:
  ...
  Si usano insieme: [p3] per la pausa interna + [stacco] per la giunzione audio.
...
================================================================================
  [inizio]
    ...
    Testo...[p1/p2/p3/b]      <- 2. testo con pause inline
    [bd] o [cap]              <- 3. pausa drammatica/capoverso (se serve)
    [e1/e2]                   <- 4. enfasi (opzionale)
...
Narrativa filosofica ([b] e [bd] per peso):
  [inizio][V1_riflessivo]
  L'anima,[p1] diceva,[p1] e la misura di tutte le cose.[b][para]
  [/V1_riflessivo][fine]

  [inizio][V1_riflessivo]
  Ma chi misura l'anima stessa?[bd][stacco]
  [/V1_riflessivo][fine]
...
Reset paragrafo con [cap]:
  [inizio][V1_calmo]
  Cosi si concluse quella lunga notte di pensieri.[p3][b][cap]
  [/V1_calmo][fine]
...
Dialogo due voci:
  [inizio][V1_serio]
  E in quest'altra parte era giusto il ragionamento?[p3][cambio]
  [/V1_serio][fine]

  [inizio][V2_riflessivo]
  Dicevamo che il giusto non puo fare ingiustizia.[p3][para]
  [/V2_riflessivo][fine]

Climax emotivo:
  [inizio][V1_arrabbiato]
  No![p1] Non e possibile![e2][bd][para]
  [/V1_arrabbiato][fine]
...
================================================================================
  1. MAI chunk con meno di 5 parole pulite (esclusi tag)
     SBAGLIATO: [inizio][V1_serio]Bene.[p2][cont][/V1_serio][fine]
     CORRETTO:  [inizio][V1_serio]Bene,[p1] diceva Critone,[p1] e ora?[p2][cont][/V1_serio][fine]
  ...
  5. [stacco] e [lungo] NON cambiano voce: creano distanza semantica nella stessa voce
...
def build_python_script(chunks, exag, cfg, temp, v1, v2, epreset, devmode="auto"):
    ...
    L = [
"# Script generato da ChatterText v2.2",
"# Pause gaussiane 6 livelli + giunzioni 7 livelli + dynamic_pause semantica",
...
"    '[pausa]':(0.50,0.05),'[pausa_lunga]':(1.20,0.10),'[silenzio]':(2.00,0.15),",
"}",
"def gp(tag): b,s=PM.get(tag.lower(),(0.40,0.05)); return max(0.05,random.gauss(b,s))",
"# GIUNZIONI 7 livelli",
"JM={'[join]':(0.00,'overlap'),'[cont]':(0.12,'smooth'),'[cambio]':(0.50,'cambio'),",
"    '[para]':(0.90,'silence'),'[stacco]':(1.40,'fade_sil_fade'),",
"    '[lungo]':(1.80,'fade_sil_fade'),'[scena]':(2.00,'hard')}",
...
"# =============================================================",
"# dynamic_pause() AVANZATA v2.2",
"# Fallback quando il chunk non ha giunzione esplicita.",
"# Scala semantica 6 livelli + gauss + moltiplicatori semantici.",
"# Valori allineati alla scala pause inline per coerenza.",
"# =============================================================",
"def dyn_pause(txt):",
"    t=txt.strip(); lo=t.lower(); ln=len(t); lc=t[-1:] if t else ''",
"    # 1. BASE dalla punteggiatura finale",
"    if t.endswith('...'): base,sig=1.50,0.15   # sospensione -> [bd]",
...
"    # 2. Scala per lunghezza (chunk lunghi meritano piu respiro)",
"    if ln>500:   base*=1.50",
"    elif ln>300: base*=1.30",
"    elif ln>150: base*=1.12",
"    elif ln<60:  base*=0.80",
"    # 3. Moltiplicatori semantici — aumentano la pausa",
...
"    # 4. Dialogo — diminuisce la pausa (le battute si susseguono piu veloci)",
"    if any(v in lo for v in DIALOG):          base*=0.75",
"    # 5. Campionamento gaussiano + clamp [0.12, 2.50]",
"    return max(0.12,min(random.gauss(base,sig),2.50))",
...
class App(tk.Tk):
    ...
    def _hdr(self,r):
        ...
        tk.Label(h,text="v2.2  |  Pause gaussiane 6 livelli  |  Giunzioni 7 livelli  |  dynamic_pause semantica",
    ...
    def _guide_sec(self,r):
        ...
        tk.Label(hr,text="Guida Tag — v2.2",font=FH2,fg=C["accent"],bg=C["bg"]).pack(side="left")
        ...
        tk.Label(lc,text="  gauss(base,sigma) -> lettura organica anti-robotica | [bd] e [cap] = NUOVO v2.2",
        ...
        for ci,(tag,col,desc) in enumerate([
            ...
            tk.Label(cell,text=desc,font=FS,fg=C["text_dim"],
                     bg=C["surface2"],padx=2,pady=2,justify="center").pack()
        tk.Label(rc,text="  [stacco],[lungo]: fade-out + silenzio + fade-in — stacco musicale tra pensieri | NUOVO v2.2",
        ...
        sb2.insert("1.0",
            ...
            "  2. Testo [p1..cap] pause inline\n"
            "  3. [e1/e2]         enfasi (opz.)\n"
            "  4. [giunzione]     join/cont/cambio\n"
            "                     para/stacco/lungo/scena\n"
            "  5. [/V1_emo]       chiusura\n"
            "[fine]")
        ...
        gb.insert("1.0",
            "Virgola, inciso         [p1]  ~0.18s\n"
            "Punto normale           [p2]  ~0.40s\n"
            "Riflessione, ; domanda  [p3]  ~0.65s\n"
            "Cambio idea, :          [b]   ~1.00s\n"
            "Rivelazione, climax     [bd]  ~1.60s  NEW\n"
            "Reset, capoverso        [cap] ~2.00s  NEW\n"
            "\n"
            "Narrazione base:        [p2]  ~0.40s\n"
            "Filosofia/riflessione:  [p3]  ~0.65s\n"
            "Passaggi profondi:      [b]+  ~1.00s+")
        gb.config(state="disabled")
        tk.Label(inner,text="Usa 'Copia Prompt AI' per dare a Claude/GPT le istruzioni complete v2.2.",
    ...
    def _footer(self,r):
        ...
        tk.Label(ft,text="2026 (c) ChatterText v2.2 by Gerardo D'Orrico  —  Gauss Pause | 7-level Join | Semantic dynpause",
