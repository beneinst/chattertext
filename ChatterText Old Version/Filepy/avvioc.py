...
for idx, par in enumerate(chunks):
    ...
    temperature=0.6,
    min_p=0.02,
    repetition_penalty=1.02,   # aggiungi questo
    top_p=0.98
    )

    audio_segments.append(wav)

# === SALVATAGGIO SEQUENZIALE con numerazione progressiva 01, 02, 03 … ===
import pathlib, os

out_dir   = pathlib.Path("1.Output")
existing  = list(out_dir.glob("cap_*"))
cap_num   = len(existing) + 1
cap_dir   = out_dir / f"cap_{cap_num:02d}"
cap_dir.mkdir(exist_ok=True)

for idx, seg_audio in enumerate(audio_segments, start=1):
    chunk_path = cap_dir / f"audiolibro_{idx:02d}.wav"
    ta.save(chunk_path, seg_audio, model.sr)
print("Capitolo salvato in:", cap_dir)